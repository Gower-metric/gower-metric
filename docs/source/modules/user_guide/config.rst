...
- ``discretization (str | None)`` - Type of discretization to be used. Possible values are `None`, `silverman` or `knn`. Default is `None`. This parameter specifies the method for adaptive discretization to reduce the disproportionate influence of categorical variables on the final result.
- ``silverman_constant (float)`` - Constant used for Silverman's rule of thumb when using KDE scaling window. Default is ``1.06`` if omitted. This parameter is only relevant if `discretization` is set to `silverman`.
...
- ``k_neighbors (int | None)`` - Number of neighbors to consider when using kNN scaling window. Default is `None`, in which case the number of neighbors will be set to the square root of the number of points. This parameter is only relevant if `discretization` is set to `knn`. Can be 1 or higher.
