...
- ``discretization (str | None)`` - Type of discretization to use. Possible values are ``None``, ``"silverman"``, or ``"knn"``. The default is ``None``. This parameter selects an adaptive discretization method to reduce the disproportionate influence of categorical variables on the final result. The approach is based on bandwidth estimation inspired by kernel density estimation (KDE), as described in Section 3 of `this publication <https://arxiv.org/pdf/2101.02481>`_. The ``"silverman"`` option closely follows the method described in the publication, whereas ``"knn"`` uses a simplified k-nearest-neighbor-inspired heuristic that estimates a global bandwidth as the median spacing between observations separated by k positions in the sorted data.
- ``silverman_constant (float)`` - Constant used for Silverman's rule of thumb when using ``"silverman"`` discretization. Default is ``1.06`` if omitted. This parameter is only relevant if `discretization` is set to `silverman`.
...
- ``k_neighbors (int | None)`` - Number of neighbors to consider when using ``"knn"`` discretization. Default is `None`, in which case the number of neighbors will be set to the square root of the number of points. This parameter is only relevant if `discretization` is set to `knn`. Can be 1 or higher.
...
- ``out_of_range (str)`` - Strategy for handling numeric and ratio-scale values that fall outside the range observed during ``fit()``. Possible values are ``"clip"`` (silently clip normalized distances to [0, 1]), ``"warning"`` (default, emit a ``UserWarning`` listing offending columns and clip), or ``"error"`` (raise a ``ValueError``). This applies to both ``transform()`` and pairwise distance calculations (``__call__``).
- ``skip_out_of_range_validation (StrictBool)`` - Determine whether to skip validation of out-of-range values. Setting to ``True`` might
    decrease overall computational cost, however it is not recommended without great care and solid data preprocessing. Default to ``False`` if omitted.
