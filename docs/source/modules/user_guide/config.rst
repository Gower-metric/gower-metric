...
- ``silverman_constant (float)`` - Constant used for Silverman's rule of thumb when using KDE scaling window. Default is ``1.06`` if omitted. This parameter is only relevant if `scale_window` is set to `kde` and `scale_window_type` is set to `silverman`.
