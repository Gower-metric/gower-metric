...
where keys are either ``int`` or ``str`` (column indices or names respectively) and values are ``str`` type. Any misnaming or
incorrect specification will lead to an error or calculation mismatch. Example below.
...
Yes, you should. For instance, missing values should be anything detectable by Python's ``math.isnan`` function, pandas' ``pd.isna`` or NumPy's ``np.isnan``.
