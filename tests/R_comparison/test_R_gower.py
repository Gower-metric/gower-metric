...
async def test_r_gower():
    ...
    assert dist_py.shape == r_dist.shape, "Shape mismatch between Python and R distances"
    assert np.allclose(dist_py, r_dist, atol=1e-6), "Distances do not match between Python and R implementations"