"""Tests for binary_symmetric_value_order parameter."""
...
class TestBinarySymmetricValueOrder:
    ...
    def test_explicit_order_truly_unseen_value_always_raises(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match=r"Value 'Maybe' in column 0 violates binary_symmetric_value_order",
        ):
    ...
    def test_explicit_order_violation_ignores_strategy(self) -> None:
        ...
        for strategy in ["error", "warning", "missing"]:
            ...
            with pytest.raises(
                ValueError,
                match=r"violates binary_symmetric_value_order",
            ):
                gower.transform(X_test)
    ...
    def test_auto_detect_raises_on_too_many_total_values(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match=r"Binary symmetric column 0 has 3 unique values total.*fitted: \['A'\], unseen: \['B', 'C'\]",
        ):
            gower.transform(X_test)
...
class TestBinarySymmetricValueOrderValidation:
    ...
    def test_validation_non_binary_column(self) -> None:
        ...
        with pytest.raises(
            ValidationError,
            match=r"contains non-binary_symmetric columns",
        ):
            Config(
