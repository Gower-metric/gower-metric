...
async def test_scikit_learn_paiwise_distances():
    ...
    assert matrix_scipy.shape == (n_rows, n_rows), "The shape of the pairwise distance matrix is incorrect."
    assert matrix_custom.shape == (n_rows, n_rows), "The shape of the custom pairwise distance matrix is incorrect."
    assert np.allclose(matrix_scipy, matrix_custom, rtol=1e-5, atol=1e-8), "The pairwise distance matrices do not match."