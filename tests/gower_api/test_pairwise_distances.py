...
@pytest.mark.asyncio
async def test_scikit_learn_paiwise_distances():
    n_rows = 500
    ...
    matrix_scikit = pairwise_distances(df, metric=_gower_distance, n_jobs=-1, ensure_all_finite=False)
    ...
    assert matrix_scikit.shape == (n_rows, n_rows), "The shape of the pairwise distance matrix is incorrect."
    assert matrix_custom.shape == (n_rows, n_rows), "The shape of the custom pairwise distance matrix is incorrect."
    assert np.allclose(matrix_scikit, matrix_custom, rtol=1e-5, atol=1e-8), "The pairwise distance matrices do not match."