...
rows = Parallel(n_jobs=cpu_count(), backend="loky", verbose=0)(
    delayed(row_distances)(i) for i in tqdm(range(n), desc="Calculating rows", unit="row")
)
...
t5 = perf_counter()
print(f"Distance matrix calculation with upper triangle speedup took {t5 - t4:.2f} seconds.")

def compare_matrices(matrix1: np.ndarray, matrix2: np.ndarray, matrix3: np.ndarray, tolerance: float = 1e-5) -> bool:
    """
    Compare the three matrices to ensure they are equal.
    """
    return np.allclose(matrix1, matrix2, atol=tolerance) and np.allclose(matrix1, matrix3, atol=tolerance) \
...
def compare_with_R(matrix_upper: np.ndarray, r_matrix: np.ndarray, tolerance: float = 1e-5) -> bool:
    """
